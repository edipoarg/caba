...
import type { CasoReportes, DataDeCasos } from "../../models/casos";
...
const ReportesMarkers = ({
  ...
  const renderMarker = (casoReportes: CasoReportes) => {
    ...
    const reportesStyle = `${styles.reportes} ${selected === Contador ? styles.selected : ""}`;
    return (
      <Marker
        key={Contador}
        longitude={longitude}
      ...
      >
        <Icons icon={"reportes_mapa"} className={reportesStyle} iconSize={"1rem"} />
      </Marker>
    );
  };

...
const DenunciaForm = () => {
  ...
  const validateForm = () => {
    const requiredFields = { fecha, hora, lugar, descripcion, nombre, telefono, email };
    const emptyFields = Object.entries(requiredFields)
      .filter(([_, value]) => !value)
      .map(([key]) => key);

    if (emptyFields.length > 0) {
      setError(`Por favor completa los campos obligatorios: ${emptyFields.join(', ')}`);
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!fecha || !hora || !lugar || !descripcion || !nombre || !telefono || !email) {
      ...
      setError('Ocurrió un error al enviar la denuncia. Por favor intenta nuevamente.');
    } finally {
      setIsSending(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="denuncia-success">
        ...
        <p>Hemos recibido tu mensaje y nos pondremos en contacto con vos si es necesario.</p>
      </div>
    );
  }
    ...
    <div className="denuncia-form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          <select 
            value={agresor} 
            onChange={(e) => setAgresor(e.target.value)}
          >
        ...
        <div className="form-group">
          ...
          />
          <small>Podes subir fotos, videos o documentos (máx. 1GB en total)</small>
        </div>
        ...
        <div className="form-checkbox">
          ...
          <label htmlFor="visibilizar">
            Autorizo que mi caso sea difundido en las redes sociales de la organización
          </label>
        </div>
